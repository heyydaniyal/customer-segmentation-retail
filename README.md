...
| Routine Generalist | 8,075 | Same grocery-heavy mix as the engaged group but ~108 distinct products and lower spend. They shop as often as the engaged group, so the gap is basket breadth, not visit frequency. |
| Promo Hunter | 6,999 | 71% of purchases made on promotion (next-highest segment: 32%). Deal-driven regardless of category. |
| Routine Young Parents | 4,921 | Cluster profile says vegetables and hygiene; basket lift says pet food, babies food, napkins and cooking oil. Highest purchase frequency in the dataset. The name follows the baskets; see the caveat under Limitations. |
...
This is almost certainly an artefact of how the synthetic course data was generated. The files are clearly linked — segment membership, which comes from `customer_info`, strongly predicts basket content (Routine Tech baskets are 30.5% personal care against 8.8% overall) — but their categories do not line up, as if each customer type had been given one spend profile and a different basket profile. It is measured in notebook 04 §3b rather than assumed either way, and it bounds what the promotions can claim: a design justified by an **association rule** stands on its own basket evidence, while one justified by a **spend profile** is a `customer_info` fact the baskets do not corroborate. Every promotion in `outputs/promotion_designs.csv` carries a `basis` column recording which it is.
...
3. **Demographic perspective** (descriptive only) — age, household, education, hour, loyalty. Computed because the brief asks for demographic analysis; the crosstab comes back exactly 100% one degree level per cluster, so the perspective *is* the `degree` column — an artefact of standardising one-hot columns, which then dominate the distance. Not used in the final segment label and not persisted with the other models.
...
- **RFM-style baseline**: quantile binning on tenure, store count and total spend. Only the monetary part is a true RFM component: the data has no recency (tenure measures time since the *first* purchase, not the last), and store count measures breadth rather than frequency. The ARI against the K-Means segmentation lands just under 0.1. That mostly reflects the product-mix splits, which a spend-and-engagement heuristic cannot see; the crosstab shows the two do share a broad engagement axis. A low ARI shows the structure is *different*, not that it is *useful* — the per-segment basket evidence in notebook 04 is what supports usefulness.
- **Compositional-data check**: the product-mix features are compositional (each row sums to 1), so K-Means was also run on a centred-log-ratio (CLR) transformation and the partitions compared. **ARI 0.417** — partial agreement, and nothing stronger. Matched by membership, 16,470 of the 21,577 customers in the raw-shares grocery-generalist cluster land in one large CLR cluster; the CLR cluster that the naming function labels `grocery_generalist` holds only 701, which is why the partitions must be compared by membership rather than by name. Notebook 03 shows the full crosstab and explains why running the naming function on the CLR labels would be circular evidence (it assigns all four names by construction and cannot fail). Raw shares are kept as the headline, with the disagreement documented as a limitation rather than resolved.
...
Per-segment Apriori with a support floor: rules need 2% support within the segment and never fewer than 30 baskets. On this data the 2% threshold always binds (the smallest segment's 1,793 baskets give 36 baskets at 2%), so the 30-basket floor is a safeguard that never activates. Each promotion design cites the specific rule(s) (with support and lift) that justify it and carries a `basis` column: `association rule`, `spend profile` (Routine Family Provisioner, where the basket evidence could not carry a design) or `mixed` (Engaged Family Provisioner, whose main mechanic comes from the spend profile).

## Limitations, and what I'd do differently

- **The grid structure is a modelling choice with a real cost.** The 3×4 two-perspective design was chosen for interpretability, and it delivers that — but it imposes a grid on the data. The Promo Hunter collapse, for example, absorbs a 1,205-customer family-provisioner group that is larger than two of the final segments. A flat clustering on the joint feature space was not run, so how much structure the grid misses is untested. If I rebuilt this, I would fit a flat clustering on the joint feature space first, characterise its clusters, and *then* decide whether a two-axis decomposition adds interpretability — rather than committing to the decomposition upfront.
...
- The demographic perspective comes out as exactly the education feature (100% per cluster). That is a modelling artefact — standardised one-hot degree columns dominate the Euclidean distance — not a property of the customers. Reported rather than hidden.
- **"Young Parents" is a basket-driven name with weak demographic support.** Kids at home are about average for the base (1.03 against 1.06), the segment is slightly older than average (57 against 55), and dog food is its top product by lift, ahead of babies food. A more neutral name would be more defensible; this one is kept for continuity and flagged here.
...
- No multiple-testing correction on the 2,334 mined rules. With that many candidates some high-lift values will be optimistic, especially in the two smallest segments, where rules rest on 36–163 and 56–97 baskets. Mitigated by an absolute 30-basket floor rather than a formal correction, and stated in notebook 04 §6.
...
|   |-- association_rules.py       Apriori per segment with a 30-basket floor
|   |-- baselines.py               RFM-style heuristic and ARI comparison
